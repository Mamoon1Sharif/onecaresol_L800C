...
const CareGivers = () => {
  ...
  const sorted = [...careGivers].sort((a, b) => {
    const aIsMamoon = a.name
      .toLowerCase()
      .includes("mamoon")
      ? 1
      : 0;

    const bIsMamoon = b.name
      .toLowerCase()
      .includes("mamoon")
      ? 1
      : 0;
  ...
  const filtered = sorted.filter((cg) => {
    ...
    if (statusFilter === "Non-Active") {
      return (
        cg.status === "Non-Active" ||
        cg.status === "Inactive"
      );
    }

    if (statusFilter === "Active") {
      const reason = caregiverUnavailableReason(
        cg as any,
        holidayEntries,
        todayStr
      );
                    ...
                    <div className="p-5 flex-1">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-1.5 shrink-0">
                          <Badge
                            className={`text-xs px-2.5 py-0.5 border-0 ${isActiveAvailable
                      ...
                      <div className="flex flex-col items-center text-center mb-4">
                        ...
                        <div className="mt-2 space-y-1.5 w-full">
                          ...
                          {cg.address && (
                            <p className="text-muted-foreground text-base leading-snug line-clamp-1 text-center">
                              {cg.address}
                            </p>
                          )}
                        </div>
                        ...
                        >
                          {visibleTags.map(
                            (tag: string) => (
                              <span
                                key={tag}
                                className={`text-xs font-semibold rounded-md px-2.5 py-1 whitespace-nowrap ${tagColors[
                                  tag
                                  ] ||
                                  "bg-muted text-muted-foreground"
                                  }`}
